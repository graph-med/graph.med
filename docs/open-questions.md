...
## rule-claim-granularity  (graph-representation.md §3.1 `thresholds`, §3.2 derived concepts, §5 `defined_by`; card #192)
**Question:** A derived concept's rule is not always its own sentence in the body text. Two forms met in the first source: one criterion sentence that lists several *alternative* rules ("Als kardiovaskuläre Hochrisikopatienten gelten Patienten mit mindestens zwei … Risikofaktoren …, ferner Patienten mit einem ASA Status von mindestens 3 und Patienten mit koronarer Herzkrankheit …"), whose thresholds cannot go on the one claim because a claim's thresholds hold at once; and a rule printed only as the parenthesis of a recommendation sentence ("Risikofaktoren für einen Harnverhalt (männliches Geschlecht, …)"), whose sentence is already a recommendation claim.
**Options:** split the alternatives into one criterion claim each (as the three amylase alternatives of the same form already are) and extract the parenthesis as a criterion claim of its own, a narrower place in the same sentence · keep one claim per sentence, the edge without thresholds, the alternatives only in the label · a rule claim may carry alternative groups of thresholds
**Leaning:** the first — a claim is one place, and §3.2 already says two ways of meeting a term are two claims; the third would put an "or" inside a claim that the edges already express. #192 extracted the two parentheses (7.7, 7.10) as criterion claims and left the 4.1 sentence whole, with its `defined_by` edge and no thresholds. (2026-09-24)
**Settled by:** the maintainer's review of #192; then the split of the 4.1 criterion, an edit with history.
