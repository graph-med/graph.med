...
## rule-claim-granularity  (graph-representation.md §3.1, §5 `defined_by`, §5.1; cards #192, #213)
**Question:** How does a derived concept reach its rule when the source prints the rule only inside a marked recommendation sentence — "Bei Risikofaktoren für einen Harnverhalt (männliches Geschlecht, tiefe anteriore Rektumresektion, Rektumexstirpation) kann …" (first source, 7.7), "… aufgrund von Risiko-Konstellationen (lange OP-Zeiten, zu erwartender hoher postoperativer Opioidverbrauch) …" (7.10); in the second source a threshold or range on a group: "(PaO2/FiO2 100-300)" (5.12), "(PaO2/FiO2 < 150 mmHg)" (5.14), "länger als 24 Stunden" (5.13), "< 4,34 mmol/L (< 7.0 g/dl)" (8.2), "(< 10.000/µl)" (8.4), "≥ 180 mg/dl (≥ 10 mmol/l)" (8.13)? §3.1 makes the sentence the unit of a box, §5.1 applies only outside marked recommendations, and `defined_by` reaches only a criterion or definition claim. A gap in the specification, not in the data.
**Until it is settled** (spec §3.1, card #213): such a rule stays in its sentence's claim, which carries no threshold; the concept it gives stays stated, its label carrying the rule as printed; and the pass that meets one names it in its pull request, so that the maintainer's route can be applied to a known list.
**Options:** a parenthesis or qualifier that states which cases a term covers becomes a criterion claim of its own, a narrower place in the box sentence, by a rule §3.1 would have to name · `defined_by` may also reach the recommendation claim whose sentence prints the rule · the concept stays stated and the page shows its label, which already carries the enumeration or the value
**Leaning:** none yet; the first keeps the edge's target a rule claim but breaks "one claim per sentence" for boxes, the second keeps the unit but makes the edge point at an instruction. Since #204 a narrower place inside a sentence exists already — every member a connective joins is a leaf claim of its own — which makes the first a smaller step than it was. (A sentence of the body text that lists alternative answers is not part of this question: §5.1 K already makes each answer a claim, as #192 did for 4.1.) (2026-09-24)
**Settled by:** the maintainer's decision on the route; then a linking pass over the listed sentences of both sources.
...
## co-editor-rejection  (graph-representation.md §3.1, §5.1; schema `claim`; card #213)
**Question:** A society that co-edits a guideline can reject one of its recommendations after the vote, and the guideline prints it: under two recommendations of the second source (p. 59, p. 61) "Die Empfehlung wurde bei Vorlage der Konsultationsfassung durch die Deutsche Gesellschaft für Anästhesiologie und Intensivmedizin e.V. (DGAI) abgelehnt.", and among the approvals (p. 149) "Empfehlungen 4.2 und 4.4 abgelehnt". Nothing in the pool can say that a recommendation was not endorsed by all who published it; the sentence is about the recommendation, not its action, so §5.1 leaves it on the page (G1), and it stays there meanwhile. The part that meets one names it in its pull request.
**Options:** leave it on the page and in the pull request · a property on each claim of the box, beside `consensus` (who dissented, as printed, with provenance on the line that prints it), since it qualifies the same vote · a `fact` claim of the sentence, linked to the box's claims by a new edge kind · an attestation-like record on the claim
**Leaning:** the second — it is printed for the box like the consensus, a reader weighing a recommendation wants the two together, and a dissent or a minority vote (*Sondervotum*) is a generic feature of consensus guidelines, not of this one; it needs a schema card. (2026-09-24)
**Settled by:** the maintainer; then a schema card before the data that records it.

## origin-and-status-labels  (graph-representation.md §3.1; schema `claim`; card #213)
**Question:** The second source prints on every box where the recommendation comes from ("SSC-Leitlinienadoption", "SSC-Leitlinienadaptation / Änderung des Empfehlungsgrades", "Adoption der S3-Leitlinie Beatmung*", "Zusätzliche DSG-Leitlinienempfehlung") and its status against the previous version ("NEU", "Modifiziert", "Bestätigt", with "Stand: 2025"). P. 3 explains the tags ("In der Leitlinie wird mit aufgeführt, ob die jeweilige Kernaussage eine Adoption bzw. Adapta-"), and its totals disagree with the boxes: "Neue (insgesamt 29) oder geänderte (insgesamt 16)" against 30 NEU, 42 Modifiziert and 16 Bestätigt printed in the boxes. No field holds either, so both stay on the page; should the pool record them?
**Options:** leave them on the page · two properties on each claim of the box, the words as printed with provenance on the box's line, and no vocabulary · a vocabulary the source declares, quoted from its method pages, as the grading scheme is · for the origin, an edge to the adopted guideline as a source entity (never extracted, card #210)
**Leaning:** the second, the words as printed and the box winning over p. 3's totals, the difference named in the pull request and never reconciled; a status is relative to the previous version and says nothing about the evidence, so it is shown, never used to rank or colour. A vocabulary waits for a second guideline that prints such tags. (2026-09-24)
**Settled by:** the maintainer, when a reader asks whether a recommendation was adopted or is new.
