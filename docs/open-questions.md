...
## board-and-packages  (ADR-0004; docs/work/README.md; the `project-board` skill)
**Question:** How does the planning board (`planning-graph.med`, Todo / In Progress / Done) relate to work packages under `docs/work/`? A card is a maintainer's planning note; a package is the reviewed scope. Today nothing links them.
**Options:** no link — the board is the maintainer's scratch space and the agent touches it only when asked · one card per package, moved by the agent as the package is claimed and merged (In Progress, Done), named by the package id · cards precede packages: a card in Todo that the maintainer promotes becomes a registered package, and the card then follows it
**Leaning:** the second once packages and cards coexist, because the mirror is cheap and the state already exists in the package file; until the maintainer says so, the first — the agent creates no cards on its own and moves only cards that name a package. Registering a package stays the maintainer's decision either way. (2026-09-19)
**Settled by:** the maintainer's first card that names a package, or a word on it.
