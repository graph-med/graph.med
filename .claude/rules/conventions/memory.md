...
| `design/grouping-axes-proposed-and-tested.md` | An axis (what a view groups by) is never fixed in the pool or the build: a person proposes one per guideline, a tool tests and reports, a linking pass asserts it with provenance, a view offers it; carried by a slot or by `axis` on `broader`. |
