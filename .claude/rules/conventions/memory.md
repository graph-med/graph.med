...
| `design/direction-legend.md` | The four-word direction (für, gegen, abwägen, Lücke) is derived from claims read in their source's grading scheme: the wording of an open grade (kann) → abwägen with a lean, any other recommendation by its direction; facts have none. |
| `design/box-granularity-per-sentence.md` | A claim is one sentence of a marked recommendation, not a box; every sentence of a box is a claim sharing its `recommendation_no`; the box's printed grade goes to its recommendation and gap-notice sentences (by wording where it prints several), its consensus to every sentence, no grade to a fact, criterion or definition. |
...
| `design/box-colour-by-direction.md` | A box is coloured by its direction (the banner's four colours); its label begins with the glyph, the grade as printed and, where the grade does not carry it in its scheme, the verb as a word; EK is marked "EK". Not by grade, not by verb; the border carries only state. |
| `design/claim-evidence-per-outcome.md` | A claim's evidence certainty is `evidence`, a list of {outcome?, key?, value?, system}, one entry per printed row in the rating system's own words, each stating an outcome or a value: a row keyed by a component, subgroup, arm, comparator, device or regimen keeps its printed `key`, never an outcome; values as printed, compared without regard to case; never a scalar, never mapped between systems, never reduced to one; provenance required like the grade; a box's rows go to each sentence with a statement of its own. |
...
| `design/body-text-rule.md` | A body-text place (sentence, footnote, table read in rows) earns a claim and an edge only by spec §5.1: gate G1–G4 in its lowest section, then `refines` → `limits` → `supplements`, first that holds; a decline is a gap notice without an edge, an evidence gap nothing; kind by form, never graded, one claim per answer; no edge between sentences of marked recommendations; an edge never decides a statement's condition. |
| `design/scope-tree-and-anchor.md` | One anchor per statement (the view's `anchor_slot`): every recommendation exactly one patient group, where its page puts it — the target group, the patients a structural measure serves, a group the guideline brings into scope under the root by `in_scope_of` — none left in a bucket (2026-09-25); a membership true only inside one guideline's scope is `in_scope_of` (optional condition concept), never `broader`; the root is declared (`scope_root`), the tree drawn by the view's first axis; anchor / subgroup / dimension / condition by four questions per statement. |
